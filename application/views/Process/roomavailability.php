...
$this->Table->Panel_MT('roomavailability', 'Process');
	...
	<tbody>
		<tr>
			<td align="left">
				...
				<strong>From </strong><input type="text" id="FDATE" name="FDATE" value="<?php echo date('d-m-Y') ?>"
					style="padding:6px" />&nbsp;<strong>To</strong>&nbsp;<input onchange="dateset()" type="text"
					id="TDATE" name="TDATE" value="<?php echo date('d-m-Y') ?>" style="padding:6px" />

				<a class="btn btn-info" onClick="Avb()">Get</a>
				&nbsp;<!--<a class="btn btn-success" onClick="Apply()" >Apply</a>-->
			</td>
		</tr>
	</tbody>
</table>
...
<script>
	...
	function dateset() {
		...
		todate = document.getElementById("TDATE").value;
	...
	function Avb() {
		...
		if (roomtype == 0) {
			...
			$.ajax({
				type: "POST",
				url: '<?php echo scs_url ?>index.php/Getval/Ability?RTYPE=' + roomtype + '&FDATE=' + $('#FDATE').val() + '&TDATE=' + $('#TDATE').val(),
				dataType: "html",
	...
	function Apply() {
		$.ajax({
			type: "POST",
			url: "<?php echo scs_url ?>index.php/Getval/AvbUpdate",
			data: $('#Avbb').serialize(),
			success: function (html) {
				scsbox(html);
			}
