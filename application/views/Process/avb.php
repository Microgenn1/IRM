...
for ($m = 0; $m < ($MO->MO + 1); $m++) {
	if ($m == 0) {
		if ($MO->MO != 0) {
			...
			$htm = "EXEC Calender '" . $Rtype . "','" . $Fdate . "','" . $sq->Yr . "/" . $sq->Mo . "/" . $sq->Day . "'," . $Cou . "," . $_SESSION['Hotel_Id'];
			$htm = $this->db->query($htm)->row();
			$Cou = $htm->Cou;
			$htm = preg_replace("[^A-Za-z0-9\-]", "", $htm->Ht . $htm->Html . $htm->Html2);
			$html .= "<td>" . $htm . "</td>";
		} else {
			$htm = "EXEC Calender '" . $Rtype . "','" . $Fdate . "','" . $Tdate . "'," . $Cou . "," . $_SESSION['Hotel_Id'];
			$htm = $this->db->query($htm)->row();
			$Cou = $htm->Cou;
			$htm = preg_replace("[^A-Za-z0-9\-]", "", $htm->Ht . $htm->Html . $htm->Html2);
			$html .= "<td>" . $htm . "</td>";
		...
		$sq = $this->db->query($sq)->row();
		$htm = "EXEC Calender '" . $Rtype . "','" . $sq->Yr . "/" . $sq->Mo . "/01','" . $Tdate . "'," . $Cou . "," . $_SESSION['Hotel_Id'];
		$htm = $this->db->query($htm)->row();
		$Cou = $htm->Cou;
		$htm = preg_replace("[^A-Za-z0-9\-]", "", $htm->Ht . $htm->Html . $htm->Html2);
		$html .= "<td>" . $htm . "</td>";
...
{
	 $htm = "EXEC Calender '" . $Rtype . "','" . $fdat . "','" . $tdat . "'," . $_SESSION['Hotel_Id'];
