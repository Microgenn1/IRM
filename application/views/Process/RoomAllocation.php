...
$this->Table->Panel_MT('RoomAllocation','Process'); 
  ...
  <tbody>
    <tr>
	  ...
	  <strong>Room Type</strong>
       ...
       <a class="btn btn-info" onClick="Avb()" >Get</a> &nbsp;<a class="btn btn-success" onClick="Apply()" >Apply</a></td> 
    </tr>
  </tbody>
</table>
...
{
	var maxval = document.getElementById(id).max;
	...
	else
	{
		...
		alert("Maximum Room Allowed is "+maxval+"/Day");
	}
}
...
function Avb()
{
	...
	else
	{
		$.ajax({
			type:"POST",
			url:'<?php echo scs_url?>index.php/Getval/Abilitychart?RTYPE='+roomtype+'&FDATE='+$('#FDATE').val()+'&TDATE='+$('#TDATE').val(),
			dataType: "html",
...
function Apply()
{
	$.ajax({
		type:"POST",
		url:"<?php echo scs_url?>index.php/Getval/AvbInsert",
		data:$('#Avbb').serialize(),
		success: function(html){
			scsbox(html);
		}
