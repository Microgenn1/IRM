...
$this->Table->Panel_R1('Order Reference','Process'); ?>
...
foreach($res->result_array() as $row)
{
	echo '<tr>';
	echo '<td>'. $cou.'</td>';
	echo '<td>'.$row['Ord_Refno'].'</td>';
	echo '<td>'.$row['room'].'</td>';
	echo '<td>'.$row['amt'].'</td>';
	echo '<td><input value="'.$row['Ord_Refno'].'" type="hidden" name="ord'.$cou.'" id="ord'.$cou.'"><input value="View" name="View" type="button" id="'.$row['Ord_Refno'].'" onclick="chk_('.$cou.')" ></td>';
	echo '</tr>';
	 $cou++;
}
